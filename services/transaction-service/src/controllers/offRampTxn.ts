...
export async function offRampTransaction(req: Request, res: Response) {
  try {
    ...
    if (accountDetails.upiId) {
      // UPI validation logic can be added here
    ...
    try {
      // Step 1: Call payment service to process off-ramp
      const paymentResponse = await offRampPayment({
        userId: String(userId),
        walletId: walletId,
      ...
      // Step 2: Debit the wallet after successful payment
      let walletResponse;
      try {
        walletResponse = await debitWallet(
          String(userId),
          sanitizedAmount,
